...
namespace HeapShot.Reader
{
	public class ReferenceNode
	{
		...
		{
			AddReference (-1, obj, 1, null);
		}
		
		void AddReference (int parentObject, int obj, int refsToRoot, string fieldName)
		{
			...
			if (refObjects.ContainsKey (obj)) {
				refObjects [obj] = refObjects [obj] + refsToRoot;
				return;
			}

			RefCount++;
			
			refObjects.Add (obj, refsToRoot);
		...
		public ArrayList References {
			get {
				...
				foreach (KeyValuePair<int,int> entry in refObjects) {
					int obj = entry.Key;
					if (inverse) {
